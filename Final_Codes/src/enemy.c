...
bool updateEnemy(Enemy * enemy, Map * map, Player * player1, Player* player2){
	...
	if (enemy->status==DEAD) return true;
...
void drawEnemy(Enemy * enemy, Point cam){
    ...
    if(enemy->status == ALIVE){
		...
		al_draw_scaled_bitmap(enemy->image, offset, 16, 16, 16, dx, dy, TILE_SIZE, TILE_SIZE, flag);
    }
...
static bool isCollision(Point enemyCoord, Map* map){
	...
	Point enemy_boundary_top_left=(Point){enemyCoord.x, enemyCoord.y};
	Point enemy_boundary_top_right=(Point){enemyCoord.x+TILE_SIZE, enemyCoord.y};
	Point enemy_boundary_bottom_left=(Point){enemyCoord.x, enemyCoord.y+TILE_SIZE};
	Point enemy_boundary_bottom_right=(Point){enemyCoord.x+TILE_SIZE, enemyCoord.y+TILE_SIZE};

	if (!isWalkable(map->map[enemy_boundary_top_left.y/TILE_SIZE][enemy_boundary_top_left.x/TILE_SIZE])) return true;
	if (!isWalkable(map->map[enemy_boundary_top_right.y/TILE_SIZE][enemy_boundary_top_right.x/TILE_SIZE])) return true;
	if (!isWalkable(map->map[enemy_boundary_bottom_left.y/TILE_SIZE][enemy_boundary_bottom_left.x/TILE_SIZE])) return true;
	if (!isWalkable(map->map[enemy_boundary_bottom_right.y/TILE_SIZE][enemy_boundary_bottom_right.x/TILE_SIZE])) return true;
